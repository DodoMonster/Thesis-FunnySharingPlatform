(function($){var ResponsiveTable=function(element,options){var that=this;this.options=options;this.$tableWrapper=null;this.$tableScrollWrapper=$(element);this.$table=$(element).find("table");if(this.$table.length!==1){throw new Error("Exactly one table is expected in a .table-responsive div.")}this.$tableScrollWrapper.attr("data-pattern",this.options.pattern);this.id=this.$table.prop("id")||this.$tableScrollWrapper.prop("id")||"id"+Math.random().toString(16).slice(2);this.$tableClone=null;this.$stickyTableHeader=null;this.$thead=this.$table.find("thead");this.$tbody=this.$table.find("tbody");this.$hdrCells=this.$thead.find("th");this.$bodyRows=this.$tbody.find("tr");this.$btnToolbar=null;this.$dropdownGroup=null;this.$dropdownBtn=null;this.$dropdownContainer=null;this.$displayAllBtn=null;this.$focusGroup=null;this.displayAllTrigger="display-all-"+this.id+".responsive-table";this.idPrefix=this.id+"-col-";this.iOS=isIOS();this.wrapTable();this.createButtonToolbar();this.setupHdrCells();this.setupStandardCells();if(this.options.stickyTableHeader){this.createStickyTableHeader()}if(this.$dropdownContainer.is(":empty")){this.$dropdownGroup.hide()}$(window).bind("orientationchange resize "+this.displayAllTrigger,function(){that.$dropdownContainer.find("input").trigger("updateCheck");$.proxy(that.updateSpanningCells(),that)})};ResponsiveTable.DEFAULTS={pattern:"priority-columns",fixedNavbar:".navbar-fixed-top",addDisplayAllBtn:true,i18n:{display:"显示项",displayAll:"显示全部"}};ResponsiveTable.prototype.wrapTable=function(){this.$tableScrollWrapper.wrap('<div class="table-wrapper"/>');this.$tableWrapper=this.$tableScrollWrapper.parent()};ResponsiveTable.prototype.createButtonToolbar=function(){var that=this;this.$btnToolbar=$('<div class="btn-toolbar" />');this.$dropdownGroup=$('<div class="btn-group dropdown-btn-group pull-xs-right" />');this.$dropdownBtn=$('<button class="btn btn-secondary dropdown-toggle" data-toggle="dropdown">'+this.options.i18n.display+' <span class="caret"></span></button>');this.$dropdownContainer=$('<ul class="dropdown-menu"/>');if(this.options.addDisplayAllBtn){this.$displayAllBtn=$('<button class="btn btn-secondary">'+this.options.i18n.displayAll+"</button>");this.$dropdownGroup.append(this.$displayAllBtn);if(this.$table.hasClass("display-all")){this.$displayAllBtn.addClass("btn-primary")}this.$displayAllBtn.click(function(){$.proxy(that.displayAll(null,true),that)})}this.$dropdownGroup.append(this.$dropdownBtn).append(this.$dropdownContainer);this.$btnToolbar.append(this.$dropdownGroup);this.$tableScrollWrapper.before(this.$btnToolbar)};ResponsiveTable.prototype.displayAll=function(activate,trigger){if(this.$displayAllBtn){this.$displayAllBtn.toggleClass("btn-primary",activate)}this.$table.toggleClass("display-all",activate);if(this.$tableClone){this.$tableClone.toggleClass("display-all",activate)}if(trigger){$(window).trigger(this.displayAllTrigger)}};ResponsiveTable.prototype.preserveDisplayAll=function(){var displayProp="table-cell";if($("html").hasClass("lt-ie9")){displayProp="inline"}$(this.$table).find("th, td").css("display",displayProp);if(this.$tableClone){$(this.$tableClone).find("th, td").css("display",displayProp)}};ResponsiveTable.prototype.setupHdrCells=function(){var that=this;that.$hdrCells.each(function(i){var $th=$(this),id=$th.prop("id"),thText=$th.text();if(!id){id=that.idPrefix+i;$th.prop("id",id)}if(thText===""){thText=$th.attr("data-col-name")}if($th.is("[data-priority]")){var $toggle=$('<li class="checkbox-row"><input type="checkbox" name="toggle-'+id+'" id="toggle-'+id+'" value="'+id+'" /> <label for="toggle-'+id+'">'+thText+"</label></li>");var $checkbox=$toggle.find("input");that.$dropdownContainer.append($toggle);$toggle.click(function(){$checkbox.prop("checked",!$checkbox.prop("checked"));$checkbox.trigger("change")});if($("html").hasClass("lt-ie9")){$checkbox.click(function(){$(this).trigger("change")})}$toggle.find("label").click(function(event){event.stopPropagation()});$toggle.find("input").click(function(event){event.stopPropagation()}).change(function(){var $checkbox=$(this),val=$checkbox.val(),$cells=that.$tableWrapper.find("#"+val+", #"+val+"-clone, [data-columns~="+val+"]");if(that.$table.hasClass("display-all")){$.proxy(that.preserveDisplayAll(),that);that.$table.removeClass("display-all");if(that.$tableClone){that.$tableClone.removeClass("display-all")}that.$displayAllBtn.removeClass("btn-primary")}$cells.each(function(){var $cell=$(this);if($checkbox.is(":checked")){if($cell.css("display")!=="none"){$cell.prop("colSpan",parseInt($cell.prop("colSpan"))+1)}$cell.show()}else{if(parseInt($cell.prop("colSpan"))>1){$cell.prop("colSpan",parseInt($cell.prop("colSpan"))-1)}else{$cell.hide()}}})}).bind("updateCheck",function(){if($th.css("display")!=="none"){$(this).prop("checked",true)}else{$(this).prop("checked",false)}}).trigger("updateCheck")}})};ResponsiveTable.prototype.setupStandardCells=function(){var that=this;that.$bodyRows.each(function(){var idStart=0;
