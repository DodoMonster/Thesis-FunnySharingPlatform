...
class DbSqlBuild {
    ...
    protected function parseKey(&$key) {//strtoupper($this->_dbDriver) == 'MYSQL'
        $key = trim($key);
        if (!preg_match('/[,\'\"\*\(\)`.\s]/', $key)) {
            $key = '`' . $key . '`';
        }
    ...
    protected function parseWhere($where) {
        ...
        if (is_string($where)) {//1,整个where是字符串
            ...
            foreach ($where as $key => $val) {
                $whereStr .='( '. $this->parseWhereItem($this->parseKey($key), $val) .' )' . $operate;//TODO parseKey的使用
            }
        ...
        if (is_array($val)) {
            if (is_string($val[0])){
                if (preg_match('/^(EQ|NEQ|GT|EGT|LT|ELT)$/i', $val[0])) { // 比较运算
                    $whereStr .= $key . ' ' . $comparison[strtolower($val[0])] . ' ' . $val[1];
                ...
                } elseif (preg_match('/IN/i', $val[0])) { // IN 运算 (NOT IN)
                    if(is_array($val[1])){//数组
                        $zone=rtrim(str_repeat('?, ',count($val[1])),', ');
                        $whereStr .= $key . ' ' . strtoupper($val[0]) . ' (' . $zone . ')';
                    }elseif(is_string($val[1])){//字符串(子查询)
    ...
    public function buildSelectSql($options = array()) {
        ...
        $sql = str_replace(array('%TABLE%', '%DISTINCT%', '%FIELD%', '%JOIN%', '%WHERE%', '%GROUP%', '%HAVING%','%UNION%', '%ORDER%', '%LIMIT%', '%COMMENT%'), 
                array(
