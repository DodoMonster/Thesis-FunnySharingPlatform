...
function getReqHmacString($p0_Cmd, $p2_Order, $p3_Amt, $p4_verifyAmt, $p5_Pid, $p6_Pcat, $p7_Pdesc, $p8_Url, $pa_MP, $pa7_cardAmt, $pa8_cardNo, $pa9_cardPwd, $pd_FrpId, $pr_NeedResponse, $pz_userId, $pz1_userRegTime,$config) {

    //include 'merchantProperties.php';
    $p1_MerId = $config['czk_merchantId'];
    $merchantKey = $config['czk_keyValue'];
...
function annulCard($p2_Order, $p3_Amt, $p4_verifyAmt, $p5_Pid, $p6_Pcat, $p7_Pdesc, $p8_Url, $pa_MP, $pa7_cardAmt, $pa8_cardNo, $pa9_cardPwd, $pd_FrpId, $pz_userId, $pz1_userRegTime,$config) {
    
    $p1_MerId = $config['czk_merchantId'];
    $merchantKey = $config['czk_keyValue'];
    $reqURL_SNDApro = $config['reqURL_SNDApro'];
   ...
   // include 'merchantProperties.php';
    ...
    #调用签名函数生成签名串
    $hmac = getReqHmacString($p0_Cmd, $p2_Order, $p3_Amt, $p4_verifyAmt, $p5_Pid, $p6_Pcat, $p7_Pdesc, $p8_Url, $pa_MP, $pa7_cardAmt, $pa8_cardNo, $pa9_cardPwd, $pd_FrpId, $pr_NeedResponse, $pz_userId, $pz1_userRegTime,$config);
    ...
    //r0_Cmd=ChargeCardDirect r1_Code=1 r6_Order=201605081801188851461751278225 rq_ReturnMsg= hmac=4ce8c3c44b3457a900206e27c2bc3149
    return $sNewString == $hmac? ['r0_Cmd'=>$r0_Cmd,'r1_Code'=>$r1_Code,'r6_Order'=>$r6_Order,'rq_ReturnMsg'=>$rq_ReturnMsg,'hmac'=>$hmac]:[];
...
function getCallbackHmacString($r0_Cmd, $r1_Code, $p1_MerId, $p2_Order, $p3_Amt, $p4_FrpId, $p5_CardNo, $p6_confirmAmount, $p7_realAmount, $p8_cardStatus, $p9_MP, $pb_BalanceAmt, $pc_BalanceAct,$config) {
...
function CheckHmac($r0_Cmd, $r1_Code, $p1_MerId, $p2_Order, $p3_Amt, $p4_FrpId, $p5_CardNo, $p6_confirmAmount, $p7_realAmount, $p8_cardStatus, $p9_MP, $pb_BalanceAmt, $pc_BalanceAct, $hmac,$config) {
    if ($hmac == getCallbackHmacString($r0_Cmd, $r1_Code, $p1_MerId, $p2_Order, $p3_Amt, $p4_FrpId, $p5_CardNo, $p6_confirmAmount, $p7_realAmount, $p8_cardStatus, $p9_MP, $pb_BalanceAmt, $pc_BalanceAct,$config))
        return true;
    else
        return false;
}
...
function arrToStringDefault1($arr){
    $Separators=',';
    $returnString = "";
    if(is_array($arr)){
        foreach ($arr as $value) {
        ...
        $returnString=substr($returnString, 0, strlen($returnString) - strlen($Separators));    
    }else{
        $returnString=$arr;
    }
    return $returnString;
