...
$apiKey = "C6foBEe4bMs9cmRWSTwxPYGS";
$secretKey = "GIW50DuThoK0fbCU3TYlVHVCuVxZsxAx";
...
//$channel = new Channel ( $apiKey, $secretKey,array ( CURLOPT_TIMEOUT => 30, CURLOPT_CONNECTTIMEOUT => 5 ) ) ;
//$channel->setApiKey($apiKey);//Channel对象初始化后，可以调用此方法重新设置apiKey。
//$channel->setSecretKey ($secretKey);//Channel 对象初始化后，可以调用此方法重新设置 secretKey。
//$result=$channel->getRequestId();//获取上次与服务器交互的 request_id，方便出错时的问题追查；如果上次调用并未与服务器交互就失败返回了，那么该函数返回0。
//$channel->setCurlOpts (array ( CURLOPT_TIMEOUT => 30, CURLOPT_CONNECTTIMEOUT => 5 ) );//Channel 对象初始化后，可以调用此方法重新设置 curl 的选项。
$result=  test_queryBindList($userId);
var_dump($result);